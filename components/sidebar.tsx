...
export default function Sidebar(props : {click?: ()=> void}) {
    ...
    return (
        <section className="flex gap-2 flex-col w-50 text-white sm:text-black">
            ...
            {path === "/dashboard/products" && <Link onClick={props.click} className="flex flex-row mb-5 w-[fit-content]" href={"/dashboard/products/importProduct"}><Image className="ml-3" src={AddProduct} width={20} height={20} alt="image" /> ایجاد محصول جدید</Link>}
            {path === "/dashboard/users" && <Link onClick={props.click} className="flex mb-5 w-[fit-content]" href={"/dashboard/users/addUser"}><Image  className="ml-3" src={AddUser} width={20} height={20} alt="image"/>ایجاد کاربر جدید</Link>}
        </section>)
}